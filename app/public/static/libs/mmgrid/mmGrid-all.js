...
!function($){
    ...
    MMGrid.prototype = {
        ...
        , _loadAjax: function(args){
            ...
            //modify by gzshangtao
            window['PYG_CURR_PAGE'] = params.page;
            that._showLoading();
            $.ajax({
                type: opts.method,
                url: opts.url,
                ...
                try{
                    //modify by gzshangtao
                    if(params.page>Math.ceil(data.totalCount/params.limit)){
                        var newargs = $.extend({}, args);
                        newargs['page'] = Math.ceil(data.totalCount/params.limit);
                        that._loadAjax(newargs);
                        return;
                    }
                    //获得root对象
                    var items = data;
