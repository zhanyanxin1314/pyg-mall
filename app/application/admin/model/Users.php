...
use app\admin\validate\Users as validate;
use think\Db;
use think\Model;
/**
 * 会员业务处理
...
class Users extends Model{
	...
	public function pageQuery(){
		...
		$rs = $this->where($where)
					->field(['userId','loginName','userName','userType','userPhone','userEmail','userScore','createTime','userStatus','lastTime'])
					->order($order)
					->paginate(input('limit/d'))
					->toArray();
	    foreach ($rs['data'] as $key => $v) {
	    	$r = PYGUserRank($v['userScore']);
