...
use \think\Model;
use think\Db;
use app\admin\validate\GoodsCats as validate;
...
class GoodsCats extends Model{
	...
	public function edit(){
		...
		$data = input('post.');
		PYGUnset($data,'catId,dataFlag,createTime');
		$validate = new validate();
		...
		$this->where("catId in(".implode(',',$ids).")")->update(['isShow' => (int)$data['isShow']]);
		if(false !== $result){
	...
	public function editName(){
		...
		if($catName=='')return PYGReturn("操作失败，商品分类名称不能为空");
		if(mb_strlen($catName)>20)return PYGReturn('商品分类名称不能超过20个字'.mb_strlen($catName));
		$id = (int)input('id');
